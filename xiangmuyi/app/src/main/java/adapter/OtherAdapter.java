...
public class OtherAdapter extends BaseAdapter {
    ...
    public String getItem(int position) {
        if (channelList != null && channelList.size() != 0) {
            return channelList.get(position%channelList.size());
        }
        return null;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String channel = getItem(position%channelList.size());
        ...
        if (!isVisible && (position%channelList.size() == -1 + channelList.size())){
            item_text.setText("");
            item_text.setSelected(true);
            item_text.setEnabled(true);
        }
        if(remove_position == position%channelList.size()){
            item_text.setText("");
        }
        return view;
    }
    ...
    public void setRemove(int position) {
        remove_position = position%channelList.size();
        notifyDataSetChanged();
        // notifyDataSetChanged();
    }
